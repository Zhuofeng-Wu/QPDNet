...
class DataAugmentation(object):
    ...
    def __call__(self, sample):
        ...
        if self.rng.binomial(1, 0.5):
            ...
            image_center = (
                self.rng.uniform(0, sample['img_right'].shape[0]),
                self.rng.uniform(0, sample['img_right'].shape[1]),
            )
            rot_mat = cv2.getRotationMatrix2D(image_center, ag, 1.0)
            sample['img_right'] = cv2.warpAffine(
                sample['img_right'], rot_mat, sample['img_right'].shape[1::-1], flags=cv2.INTER_LINEAR
            )
        ...
        sample['occu_left'] = cv2.resize(
                sample['occu_left'].astype(np.float32),
                None,
                fx=resize_scale,
                fy=resize_scale,
        ...
        if self.rng.binomial(1, 0.5):
            ...
            cx = int(self.rng.uniform(sx, sample['img_right'].shape[0] - sx))
            cy = int(self.rng.uniform(sy, sample['img_right'].shape[1] - sy))
            # sample['img_right'][cx - sx : cx + sx, cy - sy : cy + sy] = 0
            
            sample['img_right'][cx - sx : cx + sx, cy - sy : cy + sy] = np.mean(
                np.mean(sample['img_right'], 0), 0
            )[np.newaxis, np.newaxis]
            
            # update the occlusion_mask
            sample['occu_left'][cx - sx : cx + sx, cy - sy : cy + sy]=1
...
class RandomBrightness(object):
    ...
    def __call__(self, sample):
        ...
        if np.random.random() < 0.5:
            ...
            brightness2 = 2.0-brightness
            # print(brightness, brightness2)
            
            for i in range(sample.shape[0]//2):
                sample[2*i] = F.adjust_brightness(sample[2*i], brightness)
                sample[2*i+1] = F.adjust_brightness(sample[2*i+1], brightness2)
                # print(2*i, 2*i+1)
